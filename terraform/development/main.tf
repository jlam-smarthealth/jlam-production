...
resource "scaleway_instance_server" "dev_server" {
  ...
  # Storage Configuration - Preserve 20GB SBS volume
  root_volume {
    size_in_gb  = 20
    volume_type = "sbs_volume"
  }

  # IPv6 Configuration (Medical-Grade Compliance Fix)
  # Note: IPv6 disabled for security compliance

  # Security Groups
  security_group_id = scaleway_instance_security_group.dev_security.id

  # SIMPLE cloud-init - Docker only, no templates, no bullshit
  cloud_init = file("${path.module}/cloud-init-simple.yml")

  # MEDICAL-GRADE CLOUD-INIT DIAGNOSTIC - Fresh server required
  # Cloud-init only executes once on first boot - need new server for new config
  lifecycle {
    # Force server replacement for cloud-init testing
